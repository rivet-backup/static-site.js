...
export class OAuthProvider {
	...
	get oauthUrl(): string {
		const nextUrl = `${window.location.origin}${window.location.pathname}?linked=${this.id}`;
...
export class Deferred<T = void> {
	...
	constructor(startResolved: boolean = false) {
		this.reset();
...
let utils = {
	...
	formatDateLong(
		...
		multiStageDuration: boolean = false
	): string {
	...
	formatDuration(duration: number, hasSeconds: boolean = false) {
		// Decompose duration
		...
		if (hasSeconds) {
			...
			else if (!s.length) s.push(`1m`);
		}

		return s.join(' ');
	},

	formatDurationLong(duration: number, single: boolean = false, hasSeconds: boolean = false) {
		// Decompose duration
		let seconds = Math.floor(duration / 1000);
		let minutes = Math.floor(seconds / 60);
		let hours = Math.floor(minutes / 60);
		...
		return single ? s[0] : s.join(' ');
	},

	formatContentLength(bytes: number) {
		let kb = Math.floor(bytes / 1000);
		let mb = Math.floor(bytes / 1000 / 1000);
		let gb = Math.floor(bytes / 1000 / 1000 / 1000);
	...
	formatContentLengthBinary(bytes: number) {
		...
		let gb = Math.floor(bytes / 1024 / 1024 / 1024);

		if (gb > 0) return gb + ' GiB';
		else if (mb > 0) return mb + ' MiB';
		else if (kb > 0) return kb + ' KiB';
		else return bytes + ' B';
	},
	...
	formatActivity(presence: api.identity.IdentityPresence): string {
	...
	formatIdentityListName(
		...
		// Two identities in a chat, make the chat title the username of the other identity
			...
			return identities.find(identity => identity.id != currentIdentity.id).displayName;
		} else {
	...
	formatList(items: string[], truncationLength: number = 0, andSymbol: string = 'and') {
		// Render list of identities
